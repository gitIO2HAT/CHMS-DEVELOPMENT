                                ...
                                <td class="px-6 py-4 text-sm">
                                    @if($request->status == 'pending')
                                        <div class="flex space-x-2">
                                            <form method="POST" action="{{ route('admin.participation-requests.approve', $request->id) }}">
                                                @csrf
                                                <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition duration-300">
                                                    <i class="fas fa-check mr-2"></i>Approve
                                                </button>
                                            </form>
                                            <form method="POST" action="{{ route('admin.participation-requests.reject', $request->id) }}">
                                                @csrf
